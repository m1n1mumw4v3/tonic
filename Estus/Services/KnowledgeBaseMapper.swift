...
enum KnowledgeBaseMapper {
    ...
    private static func mapSupplements(_ snapshot: KnowledgeBaseSnapshot, idToName: [UUID: String]) -> [Supplement] {
        snapshot.supplements.map { s in
            ...
            // What to look for: try mechanism goalRelevance from DB, fall back to hardcoded KB
            let whatToLookFor: String = {
                let relevances = snapshot.mechanisms
                    .filter { $0.supplementId == s.id }
                    .compactMap(\.goalRelevance)
                    .filter { !$0.isEmpty }
                if !relevances.isEmpty {
                    return relevances.joined(separator: ". ")
                }
                return SupplementKnowledgeBase.allSupplements
                    .first(where: { $0.name == s.name })?.whatToLookFor ?? ""
            }()
