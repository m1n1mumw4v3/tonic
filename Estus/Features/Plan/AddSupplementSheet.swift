...
class AddSupplementViewModel {
    ...
    func load(profile: UserProfile, existingSupplements: [PlanSupplement], catalog: SupplementCatalog, drugInteractions: [DBDrugInteraction] = [], medications: [DBMedication] = [], deepProfileModules: [DeepProfileModule] = []) {
        let engine = RecommendationEngine(catalog: catalog, drugInteractions: drugInteractions, medications: medications, deepProfileModules: deepProfileModules)
...
struct AddSupplementSheet: View {
    ...
    private var deepProfileModules: [DeepProfileModule] { Array(appState.deepProfileService.completedModules.values) }
    ...
    var body: some View {
        ...
        .onAppear {
            viewModel.load(profile: profile, existingSupplements: existingSupplements, catalog: catalog, drugInteractions: drugInteractions, medications: dbMedications, deepProfileModules: deepProfileModules)
        }
    }
        ...
        if !stagedSupplements.isEmpty {
            VStack(spacing: 0) {
                ...
                CTAButton(
                    ...
                    style: .primary,
                    action: {
                        let engine = RecommendationEngine(catalog: catalog, drugInteractions: drugInteractions, medications: dbMedications, deepProfileModules: deepProfileModules)
