...
struct MedicationsScreen: View {
    ...
    var body: some View {
            ...
            VStack(spacing: 0) {
                ScrollView {
                    VStack(spacing: DesignTokens.spacing32) {
                        // Header
                        HeadlineText(text: "Are you currently taking \nany medications?")
                        ...
                        HStack(spacing: DesignTokens.spacing12) {
                            ...
                            toggleButton(label: "No", isActive: !viewModel.takingMedications) {
                                ...
                                viewModel.selectedMedicationIds = []
                                ...
                                Button {
                                    ...
                                    .overlay(
                                        RoundedRectangle(cornerRadius: DesignTokens.radiusMedium)
                                            .stroke(DesignTokens.accentLongevity, lineWidth: 1.5)
                                    )
                                }
                            }
        ...
        .sheet(isPresented: $showingMedicationPicker) {
            MedicationPickerSheet(
                selectedMedicationIds: Bindable(viewModel).selectedMedicationIds,
    ...
    /// Also removes the corresponding medication ID if using Supabase data.
    private func removePill(_ name: String) {
        if viewModel.medications.contains(name) {
            viewModel.medications.remove(name)
            // Find and remove the matching medication ID
            if let med = appState.medications.first(where: { $0.displayName == name }) {
                viewModel.selectedMedicationIds.remove(med.id)
            }
        } else {
